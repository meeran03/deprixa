...
<div id="page-wrap">
    <table width="100%">
        <tr>
            ...
            <td style="border: 0;  text-align: center" width="48%">
				</br><img src='https://barcode.tec-it.com/barcode.ashx?data=<?php echo $row->order_inv; ?>&code=Code128&multiplebarcodes=false&translate-esc=false&unit=Fit&dpi=72&imagetype=Gif&rotation=0&color=%23000000&bgcolor=%23ffffff&qunit=Mm&quiet=0&modulewidth=50' alt=''/>
			</td>
			</div>
        </tr>
    </table>
    ...
    <table id="items">
		...
		<?php
			$nums=1;
			$sumador_total=0;
			$sql = $db->query("SELECT * FROM add_courier, detail_addcourier WHERE add_courier.qid=detail_addcourier.id_add AND add_courier.id='".Filter::$id."'");

			while ($row=mysqli_fetch_array($sql))
				{
				$id=$row["id"];
				$details_description=$row['detail_description'];
				$lprice=$row['l_price'];
				$detail_qnty=$row['detail_qnty'];
				$detail_weight=$row['detail_weight'];
				$rcosttotal=$row['r_costtotal'];
				$totaltax=$row['total_tax'];
				$totalinsurance=$row['total_insurance'];

				$r_costtotal=$row['r_costtotal'];
				$precio_venta_f=number_format($r_costtotal,2);//Formatting variables
				$precio_venta_r=str_replace(",","",$precio_venta_f);//Replace the commas
				$precio_total=$precio_venta_r*$detail_qnty;
		...
		<tr class="item-row">
            <td class="<?php echo $clase;?>" style="width: 40%;" align="left" class="description"><?php echo $details_description; ?></td>
			<td class="<?php echo $clase;?>" align="center"><?php echo $detail_qnty; ?></td>
            <td class="<?php echo $clase;?>" align="center"><?php echo $core->value_weight; ?></td>
            <td class="<?php echo $clase;?>" align="right"><?php echo $detail_weight; ?></td>
        </tr>
		<?php 
	...
	?>
		<tr>
			...
			<td  align="right" class="total-value"><div id="total"><span class="price"> <?php echo $lprice; ?> </span> </div></td>
		</tr>
		<tr>
			<td  align="right" class="blank"> </td>
			<td  align="right" colspan="2" class="total-line"><?php echo $lang['inv-shipping15'] ?> +</td>
			<td  align="right" class="total-value"><div id="total"> <?php echo round_out($totaltax); ?> </div></td>
		</tr>
		<tr>
			<td  align="right" class="blank"> </td>
		...
		<tr>
			...
			<td align="right" class="total-value balance"><div class="due"> <p style="color:white;"><b><?php echo $core->currency;?> <?php echo round_out($rcosttotal); ?> </b></p></div></td>
		</tr>
    </table>
